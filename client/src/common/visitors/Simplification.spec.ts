...
const expectObject = (input: string, expected: Expression) => {
  const parsed = treeParser.value(input).accept(new Evaluation())
  let output = undefined
  expect(() => {output = parsed.accept(new Simplification())}).not.toThrow()
